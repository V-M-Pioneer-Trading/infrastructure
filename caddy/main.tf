...
locals {
  ...
  caddy_bootstrap_commands = [
    ...
    # --network host so Caddy reaches each backend on localhost:<port>. Named
    # volumes persist issued certs/account keys across container restarts and
    # redeploys, so Caddy loads the stored cert instead of re-issuing (and
    # risking Let's Encrypt rate limits). They do not survive host replacement —
    # a deliberate, rare event — after which Caddy re-issues automatically.
    "docker run -d --name caddy --restart unless-stopped --network host -v /opt/caddy/Caddyfile:/etc/caddy/Caddyfile:ro -v caddy_data:/data -v caddy_config:/config -e AWS_REGION=${var.aws_region} -e CADDY_DOMAIN=${var.backend_domain} -e ORIGIN_VERIFY_SECRET=\"$ORIGIN_VERIFY_SECRET\" caddy-route53:latest",
  ]
}
