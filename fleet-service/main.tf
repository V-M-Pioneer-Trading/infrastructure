...
locals {
  fleet_service_bootstrap_commands = [
    ...
    "docker run -d --name fleet-service --restart unless-stopped --network host -e PORT=${var.fleet_service_port} -e AGENT_SERVICE_URL=http://localhost:${data.terraform_remote_state.agent_service.outputs.agent_service_port}/api/agent -e CORS_ALLOWED_ORIGIN=${var.cors_allowed_origin} ${var.fleet_service_image}",
  ]
}
